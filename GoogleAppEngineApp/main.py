...
def home():
	...
	if request.method =='POST':
		current_deck_name=html.escape(request.form['deck_name'])
...
def study():
	...
	if datetime.now(pytz.utc) > deck_name['end_of_day']: 
...
def set_interval(card, difficulty):
	...
	if card: 
		if difficulty == "again": interval = 1
		elif difficulty == "hard": interval = interval*1.2
		elif difficulty == "good": interval = interval*1.6
		elif difficulty == "easy": interval = interval*1.9
	return interval
	
def is_end(cards, card, deckname):
	id_token = request.cookies.get("token")
	claims = google.oauth2.id_token.verify_firebase_token(id_token, firebase_request_adapter)	
	...
	today_reps = float(deck['repetitions_today'])
	max_rep = float(deck['repetitions'])
